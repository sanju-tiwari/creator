...
import { Calendar, Eye, Heart, Loader2, MessageCircle, Send, Trash2 } from "lucide-react";
...
const { useConvexQuery, useConvexMutation } = require("@/hook/use-convex-query");
...
const PostPage = ({params}) => {
  ...
  const { data: comments, isLoading: commentsLoading } = useConvexQuery(api.comment.getPostComments,{ postId });
  ...
  const { mutate: addComment, isLoading: isSubmittingComment } = useConvexMutation(api.comment.addComment);
  const deleteComment = useConvexMutation(api.comment.deleteComment);
        ...
        {commentContent ? (
         <BarLoader width={"100%"} color="#D8B4FE" />  
            ...
            <div className="space-y-4">
               ...
               <Card key={comment._id} className="bg-gray-900/50 border-gray-800 hover:border-purple-500/50">
                <CardContent className="p-6" >
