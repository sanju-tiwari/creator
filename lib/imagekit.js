export const buildTransformationUrl = (src , transformations = [])=>{
      ...
      if (transform.overlayText) { 
        const layerParams = [`l-text`,`i-${encodeURIComponent(transform.overlayText)}`,`tg-bold`,`lx-20,ly-20`];
      ...
      if (transform.gravity) {
       ...
       return params.join(",")
       }).filter((params)=> params.length > 0).join(",")
...
export const uploadToImageKit = async (file, fileName)=>{
    ...
    console.log("hey this is me response" , response )
